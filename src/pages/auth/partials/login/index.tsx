...
import { AuthLoginCredentialsStep } from "./partials/credentials";
import { AuthLoginVerificationStep } from "./partials/verification";

interface IAuthLoginFlowProps {
  changeFlow: () => void;
}

function AuthLoginFlow({ changeFlow }: IAuthLoginFlowProps) {
  ...
  return (
    <div className="w-full sm:w-5/12 ps-8 pe-6 flex flex-col items-start justify-center gap-y-0 sm:gap-y-4">
      ...
      {step === "credentials" ? (
        <div className="mx-auto flex items-center gap-x-10 pb-5 sm:pb-0">
          <span className="text-grey-600 text-[13px]">حساب کاربری ندارم</span>
          <button
            type="button"

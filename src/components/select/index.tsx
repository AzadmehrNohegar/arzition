...
function Select({
  ...
  return (
    <Listbox value={selected} onChange={setSelected}>
      ...
      >
        <Listbox.Button className="relative w-full text-start cursor-default rounded-lg bg-grey-50 py-3 ps-4 pe-10 focus:outline-none h-full min-h-[50px]">
          <span className="block truncate">{selected}</span>
          <span className="pointer-events-none absolute inset-y-0 end-1 flex items-center pr-2 text-grey-600">
            <AltArrowDown20 aria-hidden="true" />
          </span>
        </Listbox.Button>
        <Transition
          as={Fragment}
